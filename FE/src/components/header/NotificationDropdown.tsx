...
import { BoxCubeIcon, TrashBinIcon, ChatIcon, BoltIcon } from "@/icons";
...
function getNotifIcon(type: string) {
  switch (type) {
    case "agent_created":
      return <BoxCubeIcon className="h-5 w-5 text-brand-500" />;
    case "agent_deleted":
      return <TrashBinIcon className="h-5 w-5 text-error-500" />;
    case "chat_message":
      return <ChatIcon className="h-5 w-5 text-success-500" />;
    case "task_assigned":
      return <BoltIcon className="h-5 w-5 text-warning-500" />;
    default:
      ...
      >
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          {displayNotifs.length === 0 ? (
            ...
            displayNotifs.map((notif: Notification) => (
              <li key={notif.id}>
                <DropdownItem
                  onItemClick={() => handleNotifClick(notif.id, notif.is_read)}
                  className={`flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5 ${!notif.is_read
